...
df4 <- mutate(df3, Date = as.POSIXct(strptime(paste(as.Date(Date), Time), "%Y-%m-%d%H:%M:%S")))
df5 <- select(df4, -Time)
# generate png file
...
par(mfrow=c(2,2))
# plot(1,1)
plot(df5$Date, df5$Global_active_power, type = 'l', xlab = '', ylab = "Global Active Power")
# plot(1,2)
plot(df5$Date, df5$Voltage, type = 'l', xlab = "datetime", ylab = "Voltage")
# plot(2,1)
...
# plot(2,2)
plot(df5$Date, df5$Global_reactive_power, type = 'l', xlab = "datetime", ylab = "Global_reactive_power")
dev.off()